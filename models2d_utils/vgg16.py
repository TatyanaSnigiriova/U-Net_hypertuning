    ...
    class Encoder:
        ...
        def __call__(self, inputs):
            ...
            embedding = conv2d_block(
                outputs,
                n_filters=self.n_filters * 2 ** (self.n_layers - 2),  # не повышаем степень
                num_layers=num_layers,
                **self.general_conv2d_block_kwargs
            )
        ...
        def idxs_conv_stack_generator(self):
            ...
            for i_layer in range(1, 2):
                print(i_layer, self.step * i_layer + (i_layer - 1))
                yield self.step * i_layer + (i_layer - 1)
            step = self.step + 2  # + свертка и активация, начиная с 3 уровня сети
            for i_layer in range(3, self.n_layers + 1):
                print(i_layer, step * (i_layer - 1) + i_layer + 1)
                yield step * (i_layer - 1) + i_layer + 1

        def get_last_layer_idx(self):
            if self.last_max_pooling:
                return (self.step + 2) * (self.n_layers - 1) + (self.n_layers + 1) + 1
            else:
                return (self.step + 2) * (self.n_layers - 1) + (self.n_layers + 1)
    ...
    class Decoder:
        ...
        def __call__(self, embedding, conv_stack):
            ...
            for i_layer in range(2):
                up = decode2d_block(
                    outputs,
                    input_n_filters=self.n_filters * 2 ** (self.n_layers - 2),
                    **self.general_decode2d_block_kwargs
                )
                if conv_stack:
                ...
                outputs = conv2d_block(
                    up,
                    n_filters=self.n_filters * 2 ** (self.n_layers - 2),
                    **self.general_conv2d_block_kwargs
                )

            for i_layer in range(self.n_layers - 2, 0, -1):
                if i_layer == 2:
                    self.general_conv2d_block_kwargs["num_layers"] = 2
