...
from ..models2d_utils.vgg13 import *
from ..models2d_utils.base_model import *


class UNet2D(BaseCoderDecoderModel):
    model_type_name = "UNet2D"
    ...
    def __init__(
        ...
        self.encoder_part = VGG13.Encoder(
            n_layers=self.deep,
            ...
            last_max_pooling=False, ret_conv_stack=self.ret_conv_stack,
        )

        self.decoder_part = VGG13.Decoder(
            n_layers=self.deep - 1,
            n_filters=n_filters, init_seed=init_seed,
            separable_conv=separable_conv, shortcut_connection=shortcut_connection,
            bn_before_act=bn_before_act, use_conv_bias=use_conv_bias,
        ...
        if retrain_decoder:  # and pretrained_weights:
            self.pretrained_decoder_part = VGG13.Decoder(
                n_layers=self.deep - 1,
                n_filters=n_filters, init_seed=init_seed,
                separable_conv=separable_conv, shortcut_connection=shortcut_connection,
                bn_before_act=bn_before_act, use_conv_bias=use_conv_bias,
